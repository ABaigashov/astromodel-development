...
window.addEventListener("message", function(event){
  ...
  if (event.origin != 'https://astromodel.ru' && !event.origin.startsWith('http://localhost')) {
  ...
  $('#step-2 form input[type="file"]').on('change', function(e){
    ...
    importConfiguration(e, function(json){
      if(json) {
        ...
        apiRequest(apiUrl + 'api/', 'job_execute', data, function(answer){
  ...
  apiRequest(apiUrl + 'api/', 'job_list', data, function(answer){
    ...
    for(var i in answer) {
      ...
      if(answer[i].progress == -1) {
        ...
        var resultUrl = apiUrl + 'static/job/' + answer[i].user_uid + '/' + answer[i].uid + '/error.txt';
        row += '<td><a href="' + resultUrl + '" target="_blank" class="get-result" style="color: #da343f;">Показать ошибку</a></td>';
      }
        ...
        row += '<td>' + parseFloat((answer[i].progress * 100).toFixed(2)) + '%</td>';
      }
      else {
      ...
      row += '<td>' + formatDate(answer[i].date_created, 'Y/M/D H:I:S') + '</td>';
      row += '<td>' + formatDate(answer[i].date_finished, 'Y/M/D H:I:S') + '</td>';
      row += '<td><button class="btn btn-sm btn-danger btn-remove" data-uid="' + answer[i].uid + '"><i class="fa fa-times"></i></button></td>';
      row += '</tr>';
      rows += row;
    }
    $('#job-list').html(rows);
    ...
    $('.btn-remove').on('click', function(e){
      ...
      if(confirm('Вы уверены что хотите навсегда удалить задачу #' + uid + ' из вашего списка?')) {
        var data = {
          job_uniq: uid,
          session: userSession
        };
        $('#astro-pending').css('display', 'block');
        apiRequest(apiUrl + 'api/', 'job_remove', data, function(answer){
          $('#astro-pending').css('display', 'none');
          updateJobList();
        });
      }
...
function apiRequest(url, method, data, done) {
  ...
  $.ajax({
    method: "POST",
    url: url + method,
    data: JSON.stringify({data: data, timezone: date.getTimezoneOffset()}),
    dataType: 'json'
  })
  .done(function(data) {
    if(data.redirect) {
