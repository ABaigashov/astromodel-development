...
function localhost(htmlForm) {
    let fileToLoad = Array.from(
        document.getElementById("filedrop").files
    ).filter((file) => file.name == "config.yml")[0];
...
function callGenerate() {
    if (window.location.protocol === "file:") {
        ...
        div.innerHTML =
            '<input type="file" onchange="localhost()" style="display:none;" id="filedrop" webkitdirectory multiple /><label for="filedrop" style="position:absolute;width:200px;height:100px;margin:auto;left:0;top:0;right:0;bottom:0;display:flex;justify-content:center;align-items:center;background-color:#bbb;border-style:dashed">Drop problem directory</label>';
    } else {
...
function applyAttributes(object, cases, defaults) {
    for (let [key, value] of Object.entries(
        defaultsReplacement(cases, defaults)
    )) {
...
function generateNumereticInput(name, units, slots) {
    ...
    if (slots.dementional) {
        ...
        applyAttributes(
            ...
            {
                "data-config-key":
                    name + (slots.units !== undefined ? ".value" : ""),
                minimum: slots.minimum,
                maximum: slots.maximum,
                step: slots.step,
            },
...
function generateClassParameters(key, config) {
    ...
    for (let name of config.OBJECTS[key].cases) {
        ...
        if (slots.class === "dropdown") {
            ...
            let numericInput = generateNumereticInput(
                name,
                config.UNITS,
                slots
            );
            ...
            let textInput = generateTextInput(name, config.UNITS, slots, {
...
function generateAllProblemForm(problemName, config) {
    ...
    let configNameField = generateTextInput(
        null,
        null,
        {},
        {
