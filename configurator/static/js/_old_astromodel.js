...
$(function(){
	...
	$('#btn-export').on('click', function(e) {
		...
		for(var i in objects_lists) {
			...
			if(config[list_name]) {
				new_config[list_name] = config[list_name];
			}
		}
		config = new_config;
		console.log(config);
...
function UpdateTable() {
	...
	$('.action-add').on('click', function(e){
		...
		config.point_objects.push(object);
		UpdateTable();
	});
		...
		for(var i in objects_lists) {
			...
			if(config[list_name]) {
				for(var s in config[list_name]) {
					if(config[list_name][s].id === object_id) {
						config[list_name].splice(s, 1);
						break;
					}
				}
			}
...
function FillInputForm(struct, inputs_root) {
	for(var key in struct) {
		...
		if(Array.isArray(value)) {
			...
			if('value' in value) {
				$(inputs_root).find('[data-config-key="' + key + '"]').val(value.value);
			}
			else {
				for(var fn in value) {
					if(Array.isArray(value[fn])) {
						$(inputs_root).find('[data-config-key="' + key + "." + fn + '[]"]').each((index, item) => {
							if($(item).attr('type') == 'checkbox') {
								var inp_val = $(item).attr('value');
								for(var i in value[fn]) {
									if(value[fn][i] == inp_val) {
										$(item).prop('checked', true);
										break;
									}
								}
						...
						$(inputs_root).find('[data-config-key="' + fn + '[]"]').each((index, item) => {
							$(item).val(value[fn][index]);
						});
					}
				}
			}
...
function ConfigureObject(object_id, object_index) {
	...
	var object_list = $('#select-object-type option[value="' + object_type + '"]').attr('list');
	ApplyInputData(new_object, $('#' + object_type + '-object'));
...
function UpdateConfiguration(new_object) {
	...
	for(var i in objects_lists) {
		...
		if(config[list_name]) {
			for(var s in config[list_name]) {
				if(config[list_name][s].id === new_object.id) {
					config[list_name].splice(s, 1);
					break;
				}
			}
	...
	if(!config[new_object.type.list]) {
		config[new_object.type.list] = [];
	}
	config[new_object.type.list].push(new_object);
}
